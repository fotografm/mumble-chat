    ...
    case "$yn" in
        [Yy]*)
            if ! command -v apt-get &>/dev/null; then
                ...
                # A previous installation may have left the service masked.
                sudo systemctl unmask yggdrasil 2>/dev/null || true

                # Generate config if the postinst didn't create one.
                if ! sudo test -f /etc/yggdrasil/yggdrasil.conf; then
                    sudo yggdrasil -genconf \
                        | sudo tee /etc/yggdrasil/yggdrasil.conf > /dev/null
                fi

                # Seed two public peers if the Peers list is still empty.
                PEERS_ADDED=0
                if sudo grep -qE 'Peers:\s*\[\s*\]' /etc/yggdrasil/yggdrasil.conf; then
                    PEER1='tls://london.sabretruth.org:18472'
                    PEER2='tls://yggdrasil.neilalexander.dev:64648?key=ecbbcb3298e7d3b4196103333c3e839cfe47a6ca47602b94a6d596683f6bb358'
                ...
                if sudo systemctl is-active --quiet yggdrasil; then
                    ...
                    echo "  ⚠ Yggdrasil installed but service did not start."
                    echo "    Run: sudo systemctl unmask yggdrasil && sudo systemctl start yggdrasil"
                fi
                [ -n "$YGG_ADDR" ] && echo "    Your address: $YGG_ADDR"
                if [ "$PEERS_ADDED" -eq 1 ]; then
                    echo ""
                    echo "  Two public peers added to /etc/yggdrasil/yggdrasil.conf"
                    echo "  To change peers later, edit that file."
                fi
                echo ""
            fi
            ;;
